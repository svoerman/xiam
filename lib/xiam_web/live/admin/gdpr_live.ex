defmodule XIAMWeb.Admin.GDPRLive do
  ...
  def handle_event("save_consent", %{"consent" => consent_params}, socket) do
    ...
    status = case consent_params["status"] do
      "true" -> true
      "false" -> false
      _ -> false
    end
    
    consent_params = Map.put(consent_params, "status", status)
    ...
    <div class={["min-h-screen", @theme]} phx-hook="Theme" id="theme-container">
      <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-8">
          <div>
            <h1 class="text-3xl font-bold">GDPR Compliance Management</h1>
            <div class="text-sm text-muted-foreground">
              Manage user consent, data portability, and the right to be forgotten
            </div>
          </div>
          <div class="flex items-center gap-4">
            <button
              ...
              class="rounded-md p-2 cursor-pointer bg-muted hover:bg-muted ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
            >
              <span :if={@theme == "light"} class="text-muted-foreground">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M21.752 15.002A9.718 9.718 0 0118 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 003 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 009.002-5.998z" />
                </svg>
              </span>
              <span :if={@theme == "dark"} class="text-muted-foreground">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v2.25m6.364.386l-1.591 1.591M21 12h-2.25m-.386 6.364l-1.591-1.591M12 18.75V21m-4.773-4.227l-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0z" />
                </svg>
              </span>
            </button>
            <.link patch={~p"/admin"} class="text-primary hover:text-primary/80">
              ← Back to Dashboard
            </.link>
          </div>
        </div>
        ...
        <div class="md:col-span-1 bg-card rounded-lg shadow-sm border border-border overflow-hidden">
          ...
          <div class="p-4">
            ...
            >
              ...
              <%= for user <- @users do %>
                <option value={user.id} selected={@selected_user && @selected_user.id == user.id}>
                  <%= user.email %>
                </option>
              <% end %>
            </select>
        ...
        <!-- User GDPR Panel -->
        <div class="md:col-span-3 bg-card rounded-lg shadow-sm border border-border overflow-hidden">
          <%= if @selected_user do %>
                          ...
                          <%= for consent <- @user_consents do %>
                            <tr class="hover:bg-muted/50">
                              ...
                              <td class="px-6 py-4 whitespace-nowrap">
                                <span class={["px-2 inline-flex text-xs leading-5 font-semibold rounded-full", (if consent.status, do: "bg-success/20 text-success", else: "bg-destructive/20 text-destructive")]}>
                                  <%= if consent.status, do: "Granted", else: "Denied" %>
                                </span>
                              </td>
            ...
            <div class="mb-4">
              ...
              <%= if @exported_data do %>
                ...
                <div class="flex justify-end">
                  <button type="button" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2">
                    Download JSON
                  </button>
                </div>
              <% else %>
      ...
      <%= if @show_consent_modal do %>
        <div class="fixed inset-0 bg-background/80 backdrop-blur-sm flex items-center justify-center z-50">
          <div class="bg-card rounded-lg shadow-lg max-w-md w-full mx-auto p-6 border border-border">
            ...
            <.form for={%{}} phx-submit="save_consent">
              <div class="mb-4">
                ...
                >
                  ...
                  <%= for consent_type <- @consent_types do %>
                    <option value={consent_type}><%= consent_type %></option>
                  <% end %>
                </select>
              </div>
